...
def dream(layer = -1,ITERATIONS = 50):
  img_noise = np.random.uniform(size=(videoHeight,videoWidth))
  #img_noise = np.ones((videoHeight,videoWidth)) * .5
  total_image = None
  ...
  for channel in range(h[layer].get_shape().as_list()[-1]):
    ...
    img = img_noise.copy()
    img = img.reshape((1,videoHeight,videoWidth,1))
    ...
    if total_image is None:
      total_image = img.reshape((videoHeight,videoWidth))
    else:
      total_image = np.hstack((total_image,img.reshape((videoHeight,videoWidth))))
